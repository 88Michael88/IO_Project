...
* **Problem:** Czy pozwalamy wpisywać inne waluty i przeliczamy je po sztywnym kursie, czy na razie blokujemy to całkowicie?
* **Plan:** W MVP zostajemy tylko przy PLN, żeby nie komplikować bazy danych przelicznikami i API kursowym.

**2. Usuwanie Kategorii, w których są już wydatki**
Zastanawiamy się, co ma się stać, jak użytkownik będzie chciał usunąć kategorię "Samochód", ale ma w niej już wpisane 50 transakcji z przeszłości.
* **Problem:** Jeśli usuniemy kategorię, to transakcje zawisną w próżni.
* **Opcje do rozważenia:** * Zabronić usuwania kategorii, jeśli są w niej wydatki.
    * Wymusić na użytkowniku przeniesienie wydatków do kategorii "Inne" przed usunięciem.

**3. Wydatki cykliczne (Abonamenty)**
Wielu użytkowników płaci za Netflixa, Spotify czy czynsz co miesiąc tyle samo. W tej chwili w wymaganiach mamy tylko ręczne dodawanie transakcji.
* **Problem:** Ręczne dodawanie czynszu co miesiąc jest męczące.
* **Decyzja:** To dość skomplikowane w implementacji. Z racji ograniczonego czasu, temat ten trafia na listę "Może" i raczej nie wejdzie do MVP, chyba że zostanie nam czas pod koniec projektu.

**4. Archiwizacja i ładowanie danych**
W wymaganiach niefunkcjonalnych zdefiniowaliśmy czasy ładowania (poniżej 200ms). Jak użytkownik będzie korzystał z apki przez 3 lata, to nazbiera tysiące transakcji.
* **Do ustalenia:** Czy przy pobieraniu historii transakcji (SELECT) musimy wprowadzić stronicowanie (ładowanie po 20 sztuk) od samego początku, czy na start wystarczy ładowanie wszystkiego naraz? Musimy to przetestować na etapie prototypu bazy.
